...
vfloat16mf4_t test_vunzipo_v_f16mf4_m(vbool64_t vm, vfloat16mf2_t vs, size_t vl) {
  return __riscv_vunzipo(vm, vs, vl);
}

vfloat16mf2_t test_vunzipo_v_f16mf2_m(vbool32_t vm, vfloat16m1_t vs, size_t vl) {
  return __riscv_vunzipo(vm, vs, vl);
}
...
vfloat32mf2_t test_vunzipo_v_f32mf2_m(vbool64_t vm, vfloat32m1_t vs, size_t vl) {
  return __riscv_vunzipo(vm, vs, vl);
}
...
/* { dg-final { scan-assembler-times {vseti?vli\s+[a-z0-9]+,\s*[a-z0-9]+,\s*e[0-9]+,\s*mf?[1248],\s*t[au],\s*m[au]\s+vunzipo\.[ivxfswum.]+\s+} 96 } } */
